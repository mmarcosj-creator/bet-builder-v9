...
APP_VERSION="V15.0-CLEAN-GATUNO"
DATA_DIR=Path("app_data_v15"); DATA_DIR.mkdir(exist_ok=True)
...
def refresh():
    ...
    atomic_json({"version":APP_VERSION,"generated_at":datetime.now(base.TZ_PERU).isoformat(),
                 ...
                 "inserted":rec.get("insertados",0)},META_FILE)
    return matches,markets,metrics,hist

def signal(x):
    return {"VERDE":"🟢","AMARILLO":"🟡","ROJO":"🔴"}.get(str(x).upper(),"⚪")

st.markdown(f"# 🐾 Gatuno V15 Clean\n**Motor limpio y auditable · {APP_VERSION}**")
...
if not ready:
    st.info("🐾 Primera ejecución de la instalación limpia. No reutiliza app_data ni cachés antiguas.")
    if st.button("🐾 GENERAR PRONÓSTICOS V15",type="primary",use_container_width=True):
        try:
            with st.status("Construyendo motor limpio…",expanded=True):
                refresh()
            st.success("Motor V15 listo.")
            st.rerun()
        except Exception as e:
            st.error("El motor no terminó.")
            st.code(str(e)); st.code(traceback.format_exc())
    st.stop()
