...
VERSION = "V10.4.1-GATUNO-ADAPTATIVO"
...
def _runtime_limits(fast_mode: bool):
    ...
    names = {
        ...
        "CONTEXT_DAYS": 90,
        "CONTEXT_EVENTS_PER_COMP": 7,
        "CONTEXT_MAX_SUMMARY_CALLS": 72,
        "MAX_FUTURE_SUMMARY_CALLS": 28,
        "MAX_WEATHER_CALLS": 28,
    }
...
def _run_v10_impl(progress: Any = None) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.Timestamp, pd.Timestamp]:
    ...
    _notify(progress, "Descargando historico compacto y verificable…")
    ...
    _notify(progress, "Calibrando mercados sin detener todo si uno carece de muestra…")
    bundle = train_models(dataset)
    schedule_index = base.construir_schedule_index(history)

    _notify(progress, "Consultando partidos desde este momento en adelante…")
