...
def resolve_pending():
    # This function intentionally uses the forecast core's ESPN summary helper.
    import bet_builder_v8_1_robust as base
    hist = load_history()
    if hist.empty:
        return hist, {"cerrados":0, "pendientes":0, "errores":[]}
    closed, errors = 0, []
    for idx, r in hist[hist["Estado"].astype(str).eq("PENDIENTE")].iterrows():
        try:
            kickoff = pd.to_datetime(r["KickoffUTC"], utc=True, errors="coerce")
            if pd.isna(kickoff) or kickoff > pd.Timestamp.now(tz="UTC") - pd.Timedelta(minutes=20):
                continue
            league = base.COMPETICIONES.get(str(r.get("CompKey","")),{}).get("espn")
            if not league or not r.get("EventID"): continue
            summary = base.resumen_evento_espn(league, str(r["EventID"]), cache_hours=0)
            ok, actual = _actual_from_summary(r, summary)
            if ok is None:
    ...
    return hist, {"cerrados":closed, "pendientes":int((hist["Estado"]=="PENDIENTE").sum()), "errores":errors}
